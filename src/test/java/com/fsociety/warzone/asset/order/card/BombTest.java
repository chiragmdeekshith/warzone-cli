...
class BombTest {
    ...
    void setUp() {
        ...
        // Ensure the player has a Bomb card
        d_player1.getHandOfCards().drawSpecificCard(HandOfCards.Card.BOMB);
    ...
    void countryValidationTest() {
        ...
        d_player2.getHandOfCards().drawSpecificCard(HandOfCards.Card.BOMB);
        l_attack.bomb(1); // Player does not own neighbouring country
        l_attack.bomb(4); // Player cannot bomb their own country
        l_attack.bomb(5); // Country does not exist
        assertNull(d_player2.nextOrder());
    }

    /**
     * Test to ensure the bomb has the desired effect.
     */
    @Test
    void bombEffectTest() {
        Phase l_attack = new Attack();
        IssueOrder.setCurrentPlayer(d_player1);
        d_playMap.updateCountry(4, 8);
        IssueOrder.d_availableTroopsOnMap = new HashMap<>();
        for (Country l_country : GameplayController.getPlayMap().getCountries().values()) {
            IssueOrder.d_availableTroopsOnMap.put(l_country.getCountryId(), l_country.getArmies());
        }
        ...
        assertEquals(d_playMap.getCountryState(4).getArmies(), 4);
    }

    /**
     * Test to make sure the issuing player has the Bomb card.
     */
    @Test
    void hasBombCardTest() {
        Phase l_attack = new Attack();
        IssueOrder.d_availableTroopsOnMap = new HashMap<>();
        for (Country l_country : GameplayController.getPlayMap().getCountries().values()) {
            IssueOrder.d_availableTroopsOnMap.put(l_country.getCountryId(), l_country.getArmies());
