package com.fsociety.warzone.models.command;
...
public class ExecuteOrder {
    ...
    public static void executeOrders(ArrayList<Player> p_players) {
        ...
        for (Order l_order : l_orders) {
            if (!GameEngineController.getPlayerNameMap().get(GameEngineController.getPlayerNameFromId(l_order.getIssuerId())).isEliminated()) {
                l_order.execute();
                if (GameEngineController.checkWinCondition()) {
                    GameEngineController.setGameWon();
                    return;
                }
            }
        }
