...
import com.fsociety.warzone.models.order.Advance;
import com.fsociety.warzone.models.order.card.*;
...
public class Attack extends MainPlay {
    ...
    public void advance(int p_sourceCountryId, int p_targetCountryId, int p_troopsCount) {
        ...
        boolean l_playerOwnsCountry = GameEngineController.getPlayMap().getCountryState(p_sourceCountryId).getPlayerId() == IssueOrder.getCurrentPlayer().getId();
        boolean l_countriesAreDifferent = p_sourceCountryId != p_targetCountryId;
        boolean l_countriesAreNeighbours = GameEngineController.getPlayMap().getNeighbours().get(p_sourceCountryId).contains(p_targetCountryId);
        boolean l_sourceCountryHasEnoughTroops = IssueOrder.d_availableTroopsOnMap.get(p_sourceCountryId) >= p_troopsCount;
        boolean l_sourceCountryExists = GameEngineController.getPlayMap().getCountryState(p_sourceCountryId) != null;
        boolean l_targetCountryExists = GameEngineController.getPlayMap().getCountryState(p_targetCountryId) != null;
    ...
    public void bomb(int p_targetCountryId) {
        boolean l_isEnemyCountry = GameEngineController.getPlayMap().getCountryState(p_targetCountryId).getPlayerId() != IssueOrder.getCurrentPlayer().getId();
        boolean l_isNeighbour = GameEngineController.getPlayMap().isNeighbourOf(p_targetCountryId, IssueOrder.getCurrentPlayer().getId());
        boolean l_targetCountryExists = GameEngineController.getPlayMap().getCountryState(p_targetCountryId) != null;
    ...
    public void negotiate(String p_targetPlayerName) {
        ...
        Player l_enemy = GameEngineController.getPlayerNameMap().get(p_targetPlayerName);
